...
@bind newton_args PlutoUI.combine() do Child
	md"""
	$(Child(TextField(default="0"))) Initial Guess
	
	$(Child(TextField(default="0.05"))) Tolerance
	"""
end

# ╔═╡ 259aeb4d-f08f-4a2c-b676-e34c159f39aa
@bind display_extra PlutoUI.combine() do Child
	md"""
	$(Child(CheckBox(default=false))) Display table of optimization

	$(Child(CheckBox(default=false))) Plot optimal values
	"""
end

# ╔═╡ 80eb2733-b374-4965-aee6-4c3f4bc338d1
if display_extra[2]
	@bind plot_args PlutoUI.combine() do Child
		md"""
		$(Child(TextField(default="-3"))) Min
		
		$(Child(TextField(default="3"))) Max
		"""
	end
end
...
if values !== nothing
	...
	F_x′ = polynomial_derivative(F_x)
	F_x″ = polynomial_derivative(F_x′)
	
	fx_latex = poly_to_latex(F_x)
    fx′_latex = poly_to_latex(F_x′)
	fx″_latex = poly_to_latex(F_x″)
...
begin 
	x_1 = parse(Float64, newton_args[1])
	ϵ = parse(Float64, newton_args[2])
	optimal_value = newton_raphson_method(F_x, x_1, ϵ)
	optimal_solution = evaluate_at(F_x, optimal_value)
	Markdown.parse("""
	## Newton-Raphson Method for Optimization
	
	Polynomial: ``f(x) = $fx_latex``
	
	Inital Guess: ``x_{1} = $x_1``
...
try 
	r = (
		parse(Float64, plot_args[1]),
		parse(Float64, plot_args[2])
	)
	if display_extra[1]
...
[sources]
Newts = {rev = "canon", subdir = "Newts", url = "https://github.com/computerscience-person/CCS239-Final-Project.git"}
...
[[deps.Newts]]
deps = ["DataFrames", "Plots"]
git-tree-sha1 = "90299c590223ce31510cc668ba44bbda9638be51"
repo-rev = "canon"
repo-subdir = "Newts"
